...
class SegDirectoryIterator(Iterator):
    ...
    def __init__(self, file_path, seg_data_generator,
        ...
        self.nb_sample = len(self.data_files)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.target_size:
            ...
            #batch_y = np.zeros((current_batch_size,2) ,dtype=int)
        ...
        for i, j in enumerate(index_array):
            ...
            img_file_format = 'img'
            img = load_img(os.path.join(self.file_path, data_file),
            ...
            """
            x = self.seg_data_generator.random_transform(x)
        ...
        if self.save_to_dir:
            for i in range(current_batch_size):
                ...
                label.save(os.path.join(self.save_to_dir,
                                        'label_' + fname + '.png'))
        # return
        ##########batch_y for not real softmax
        batch_x = preprocess_input(batch_x)
        ones = np.ones((5,1))
        zeros = np.zeros((5,1))
        batch_y = np.concatenate([ones,zeros],1)
...
class SegDataGenerator(object):
    ...
    def random_transform(self, x):
        ...
        if self.crop_mode == 'center':
            x = center_crop(x, self.crop_size, self.data_format)
        elif self.crop_mode == 'random':
            x = random_crop(x, self.crop_size, self.data_format)
